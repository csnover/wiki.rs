...
testframework.types.Normal = {
    ...
    exec = function ( func, args )
        ...
        if table.remove( got, 1 ) then
            ...
            if string.sub( tostring(got[1]), 1, 6 ) == 'SKIP: ' then
                error( got[1], 0 )
            end
            got = string.gsub( tostring(got[1]), '^%S+:%d+: ', '' )
            return 'ERROR: ' .. got
        end
    end
}
...
function testframework.getTestProvider( tests )
    ...
    for _, t in ipairs(tests) do
        local type = testframework.types[t.type or 'Normal']
        local result = type.exec( t.func, t.args or {} )
        local expect = type.format(t.expect)
        if deepEquals(result, expect) then
            print("OK: " .. t.name)
        else
            fails = fails + 1
            print("Fail: " .. t.name)
