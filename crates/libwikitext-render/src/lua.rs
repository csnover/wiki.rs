...
use libwikitext_lua::{WallTime, prelude::*};
use libwikitext_lua_gpl::HostCall;
use libwikitext_lua_gpl::UnstripMode;
...
use piccolo::{
    ...
    StashedString, StashedTable, StashedValue, TypeError, thread::BadExecutorMode,
};
use std::{borrow::Cow, sync::Arc, time::Instant};

/// The concrete type used by the renderer for [`LuaEngine`](libwikitext_lua_gpl::LuaEngine).
pub type LuaEngine =
    libwikitext_lua_gpl::LuaEngine<Arc<dyn IDatabase>, Pin<&'static StackFrame<'static>>>;
...
pub type TitleLibrary = libwikitext_lua_gpl::TitleLibrary<Arc<dyn IDatabase>>;
/// The concrete type used by the renderer for [`UriLibrary`](libwikitext_lua_gpl::UriLibrary).
pub type UriLibrary = libwikitext_lua_gpl::UriLibrary<'static, Arc<dyn IDatabase>>;
...
pub(super) fn new_vm<'config>(
    ...
    let (db, parser) = unsafe {
        (
            core::mem::transmute::<&Arc<dyn IDatabase>, &Arc<dyn IDatabase>>(db),
