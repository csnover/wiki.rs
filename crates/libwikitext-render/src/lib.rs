...
/// Strip a graf wrapper generated by `GrafWrapper` from the given `html`.
///
/// This is `Parser::stripOuterParagraph`.
#[must_use]
pub fn strip_graf(html: &str) -> &str {
    html.strip_prefix("<p>")
        .map(|c| c.strip_suffix('\n').unwrap_or(c))
        .and_then(|c| c.strip_suffix("</p>"))
        .map_or(html, |c| c.strip_suffix('\n').unwrap_or(c))
}
