...
struct Limits {
    ...
    db_cache: usize,
    /// Renderer limits.
    renderer: libwikitext_render::Limits,
...
impl core::fmt::Display for Limits {
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        ...
        let vm_total_mem = self.renderer.vm_total_mem as f64 / 1024.;
        ...
        writeln!(f, "  VM time (per call):     {:?}", self.renderer.vm_time)?;
        Ok(())
    }
}
...
/// Creates a [`LoadMode`] from a string.
fn parse_load_mode(s: &str) -> Result<LoadMode, LoadModeError> {
    if s.eq_ignore_ascii_case("base") {
        Ok(LoadMode::Base)
    } else if s.eq_ignore_ascii_case("template") {
        Ok(LoadMode::Template)
