...
/// # Errors
///
/// * The arguments on the stack are incorrect
/// * `pattern` is an invalid pattern string
/// * `s` or `pattern` cannot be converted to the backing representation (e.g. a
///   non-UTF-8 string for a `str` backing type)
/// * `init` is out of range of the machine pointer type
    ...
    let result = if plain == Some(true) {
        if pattern.is_empty() {
            let pos = {
                ...
                match init0 {
                    Some(init @ 1..) => usize::try_from(init - 1)?.min(len),
                    Some(init @ ..=-1) => len.saturating_add_signed(isize::try_from(init)?),
                    _ => 0,
                }
            };
            Some((pos, pos, vec![]))
...
///
/// * The arguments on the stack are incorrect
/// * `s` cannot be converted to the backing representation (e.g. a non-UTF-8
///   string for a `str` backing type)
