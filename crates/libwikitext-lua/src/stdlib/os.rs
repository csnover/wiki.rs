...
use crate::{WallTime, prelude::*};
use gc_arena::Rootable;
use libphp_rs::{DateTime, DateTimeZone, format_date_strftime};
...
pub fn load_os(ctx: Context<'_>) {
    ...
    os.set_field(
        ...
        Callback::from_fn(&ctx, move |ctx, _, mut stack| {
            ...
            let time = time
                .map_or_else(
                    || Ok(ctx.singleton::<Rootable![WallTime]>().get()),
    ...
    os.set_field(
        ...
        Callback::from_fn(&ctx, move |ctx, _, mut stack| {
            let time = if let Some(options) = stack.consume::<Option<Table<'_>>>(ctx)? {
                ...
                ctx.singleton::<Rootable![WallTime]>()
                    .get()
                    .unix_timestamp()
            };
