...
local function checkerror (msg, f, ...)
  local s, err = pcall(f, ...)
  assert(not s and string.find(tostring(err), msg))
end
...
local function PU (p)
  ...
  p = string.gsub(p, "([\0-\x7F\xC2-\xF4][\x80-\xBF]*)%?", function (c)
    return string.gsub(c, ".", "%0?")
  end)
  -- change '.' to utf-8 character patterns
  p = string.gsub(p, "%.", "[\0-\x7F\xC2-\xF4][\x80-\xBF]*")
  return p
end
...
local function malform (p, m)
  ...
  assert(not r and string.find(tostring(msg), m))
end
