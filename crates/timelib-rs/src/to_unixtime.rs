...
use super::{
    DateTime, DateTimeBuilder, Error, Keyword, Relatime, Special, Timezone, WeekdayBehavior,
    Weekdays,
};
use time::{Date, Duration, Month, PrimitiveDateTime, Time, UtcOffset, Weekday};

impl<'a> DateTimeBuilder<'a> {
    /// Generates a Unix timestamp from this builder, using `other` to fill
    /// any fields which were not previously set.
    pub fn build(mut self, other: Option<Self>) -> Result<DateTime<'a>, Error> {
        ...
        let datetime = PrimitiveDateTime::new(date, time).saturating_add(time_delta);

        let offset = match self.offset.as_ref() {
            Some(Timezone::Offset(offset)) => UtcOffset::from_whole_seconds(*offset)?,
            Some(tz @ Timezone::Alias(_)) => {
        ...
        Ok(DateTime {
            date: datetime.assume_offset(offset),
            tz: self.offset.unwrap_or(Timezone::Offset(0)),
        })
    }
