//! Generates the time zone alias list from tzdb.
...
/// Emits Rust code for a list of time zone aliases, sorted for binary search.
fn build_tz_list(out: &mut impl Write) {
        ...
        .filter(|name| accepted_tz_name.is_match(name))
    {
        ...
        for local in zone.local_time_types() {
            ...
            let key = (
                local.time_zone_designation().to_owned(),
                local.ut_offset(),
                local.is_dst(),
            );
...
/// The time zone alias priorities for ???
