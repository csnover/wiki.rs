...
peg::parser! { grammar expr() for str {
  ...
  = "0" ['x'|'X'] d:$(hex_digit()+)
  {
    // Clippy: Precision loss would also occur in ECMAScript.
...
impl From<&VmValue<'_>> for i64 {
    // Clippy: The only use of this conversion is in date creation, and
    // truncation is the defined behaviour per ECMAScript 2026 §§21.4.1.27-28.
...
const FUNCTIONS: phf::Map<&'static str, &'static dyn Callable> = phf::phf_map! {
    ...
    "parseInt" => &wrap_str(|value, radix: Option<f64>| {
        // Clippy: If the radix is out of range, the input was bogus, and the
        // result will become NaN.
        #[allow(clippy::cast_precision_loss, clippy::cast_possible_truncation, clippy::cast_sign_loss)]
        let radix = radix.and_then(|radix| {
            if radix.is_nan() { None } else { Some(radix as u32) }
        });
        // Clippy: The same precision loss would happen in ECMAScript.
        #[allow(clippy::cast_precision_loss)]
...
impl<'s> VmValue<'s> {
    ...
    fn get(&self, key: &VmValue<'s>) -> VmValue<'s> {
        match self {
            Self::Array(values) => {
                // Clippy: Wacky keys will just cause a null return value.
    ...
    // Clippy: Truncation is deliberate.
    #[allow(clippy::cast_possible_truncation)]
    #[inline]
