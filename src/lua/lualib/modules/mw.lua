...
local function makePackageModule( env )
	...
	packageModuleFunc(env)

	-- Make a loader function
	local function loadPackage( modName, env )
		local init
		...
		elseif packageCache[modName] == nil then
			...
			if lib ~= nil then
				...
				init = php.loadPackage( modName, env )
				if init == nil then
...
--- Set up a cloned environment for execution of a module chunk.
function mw.makeEnv()
	...
	-- This is necessary for modules which are created by `require`
	mw.resetFrame(env)

	return env
end

--- Resets the stack frame of a module.
function mw.resetFrame( env )
...
function mw.executeFunction( chunk, name, env )
	local frame = mw.resetFrame(env)
...
local function validateData( d, seen )
	seen = seen or {}
	local tp = type( d )
	if tp == 'nil' or tp == 'boolean' or tp == 'number' or tp == 'string' or tp == 'userdata' then
		return nil
	elseif tp == 'table' then
		if seen[d] then
			return nil
...
function mw.loadData( module )
	...
	if not data then
		...
		data = require( module, mw.makeEnv(nil, newFrame( 'empty' )) )
